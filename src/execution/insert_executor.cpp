...
void InsertExecutor::Init() {
  if (!is_raw_insert_) { // we have a child plan that provides tuples
...
auto InsertExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  if (!is_raw_insert_) {
    ...
    if (values_iter_ == plan_->RawValues().cend()) { //已经没有数据可插入
      res = false;
    } else {
  ...
  if (res) {
    ...
    //给新插入的这一行数据加上写锁
    lockmanager->LockExclusive(transaction, insert_rid);                     
    //更新索引
    Tuple key_tuple;
