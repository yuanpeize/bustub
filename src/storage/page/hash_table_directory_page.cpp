...
auto HashTableDirectoryPage::GetGlobalDepthMask() -> uint32_t {
  //mask用于截取hash值的后global位
  //eg:golal_depth=3 mask=111
...
auto HashTableDirectoryPage::GetBucketPageId(uint32_t bucket_idx) -> page_id_t { 
  //buket_page_ids_记录了bucket_index和bucket_page_id之间的映射关系
...
auto HashTableDirectoryPage::CanShrink() -> bool {  
  //当所有的local_depth都比global_depth，此时gloal_depth可以减一
  uint32_t dir_size = 1 << global_depth_;
  for (uint32_t index = 0; index < dir_size; index++) {
    //local_depths_记录了每一个bucket的local_depth
...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {  
  // eg: local_depth = 3
  //     bucket_index = 011
  //     new_bucket_index = 111
  // 111桶是011桶对应的桶，即就是将桶的最高位取反
  uint32_t local_depth = GetLocalDepth(bucket_idx);
  uint32_t local_mask = GetLocalDepthMask(bucket_idx);
  
  if (local_depth == 0) {
    //local_depth=0说明只有一个bucket，返回自身
    return 0;
  }
  //eg:local_depth=3 bucket_index=011
  //   1<<local_depth = 100
  //   011^100 (给最高位取反) => 111
